...
public class GtsamInterface {
	...
	private static final DataLog logfile = new DataLog("/U/logs_matt",
			"matt_" + System.currentTimeMillis() + ".wpilog");
	...
	static void setup() {
		...
		m_ntEntryLogger = NetworkTableInstance.getDefault().startEntryDataLog(logfile, "/gtsam_meme", "/gtsam_meme");
	...
	private static class CameraInterface {
		...
		public TagDetection undistort(TagDetection distorted) {
			...
			Calib3d.undistortImagePoints(mat, mat, cameraCal.cameraMat, cameraCal.distCoeffs,
	...
	public GtsamInterface(List<String> cameraNames) {
		...
		odomPub = NetworkTableInstance.getDefault().getStructTopic("/gtsam_meme/input/odom_twist", Twist3d.struct)
				.publish(PubSubOption.sendAll(true), PubSubOption.keepDuplicates(true));
		guessPub = NetworkTableInstance.getDefault()
				.getStructTopic("/gtsam_meme/input/pose_initial_guess", Pose3d.struct)
				.publish(PubSubOption.sendAll(true), PubSubOption.keepDuplicates(true));
		tagLayoutPub = NetworkTableInstance.getDefault().getStringTopic("/gtsam_meme/input/tag_layout")
	 ...
	 * @param intrinsics
	 *            Camera intrinsics in standard OpenCV format. See:
	 *            https://docs.opencv.org/4.x/d9/d0c/group__calib3d.html
	 * @param optional
	 *            Camera distortion coefficients, of length 4, 5 or 8
	 */
	public void setCamIntrinsics(String camName, Optional<Matrix<N3, N3>> intrinsics,
			Optional<Matrix<N5, N1>> optional) {
		if (intrinsics.isEmpty() || optional.isEmpty()) {
		...
		cam.camIntrinsicsPublisher.set(new double[]{intrinsics.get().get(0, 0), intrinsics.get().get(1, 1),
				intrinsics.get().get(0, 2), intrinsics.get().get(1, 2),});

		if (cam.cameraCal != null) {
			cam.cameraCal.release();
		}
		cam.cameraCal = new CameraCalibration(intrinsics.get(), optional.get());
	}
	 ...
	 *
	 * @param odomTime
	 *            The time that the odometry twist from last iteration was collected
	 *            at, in microseconds. WPIUtilJNI::now is what I used
	 * @param odom
	 *            The twist encoding chassis movement from last timestamp to now. I
	 *            use SwerveDriveKinematics::toTwist2d
	 */
	...
	/**
	 * @param guess
	 *            An initial guess at robot pose from solvePNP or prior knowledge.
	 */
	...
	public void sendLayout(AprilTagFieldLayout layout) {
		try {
			...
			DriverStation.reportError("Error serializing tag layout: " + e.getMessage(), e.getStackTrace());
		}
	}
	 ...
	 *            The name of the camera
	 * @param tagDetTime
	 *            The time that the frame encoding detected tags collected at, in
	 *            microseconds. WPIUtilJNI::now is what I used
	 * @param camDetectedTags
	 *            The list of tags this camera could see
	 * @param robotTcam
	 *            Transform from robot to camera optical focal point. This is not
	...
	public void sendVisionUpdate(String camName, long tagDetTime, List<TagDetection> camDetectedTags,
		...
		var tags = camDetectedTags.stream()
				// .map(it -> cam.undistort(it))
				.collect(Collectors.toList()).toArray(new TagDetection[0]);
